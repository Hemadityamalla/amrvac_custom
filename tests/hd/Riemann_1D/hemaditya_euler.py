...
def euler(Ncells, CFL, ics, tfinal, gamma):
   ...
   '''
    def convertToPrimitives(ufield):
        data = ufield.get_cc()
        return np.array([data[:,0], returnVelocity(data), returnPressure(data)]).T
    ...
    nvars = 3
    #Initial conditions
    ics = convertToConservatives(ics)
    ...
    #exactSolution(ics, tFinal)
    #plt.show()
    #print(convertToPrimitives(u).shape)
    return [x.x_cc, convertToPrimitives(u)]
